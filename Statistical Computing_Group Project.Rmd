...
# First insert the student marks
...
hist(marks, probability = TRUE, col = "lightblue", main = "Histogram of Marks")
...
```{r}
# 1.2. Analytical form of the distribution
...
## 1.3 Generate a random sample of same size
...
# Generate random sample from the fitted Normal distribution
simulated_sample <- rnorm(n, mean = mu, sd = sigma)
simulated_sample
# Plot simulated data
hist(simulated_sample, probability = TRUE,
     col = "lightgreen",
     main = "Simulated Normal Sample",
     xlab = "Marks")
...
```{r}
# Goodness-of-Fit Test by using Kolmogorov–Smirnov test

# Kolmogorov–Smirnov test
ks.test(simulated_sample, "pnorm", mean = mu, sd = sigma)
...
The simulated sample fits the Normal distribution

The model is appropriate for the marks
```{r}

# QUESTION 2: Multivariate Normal from rock Dataset
#
############################################################

data(rock) # Load the rock dataset
str(rock) # sTRUCTURE OF Dataset

library(MASS) # Load MASS package for multivariate normal generation

# Compute mean vector and covariance matrix
mu_rock <- colMeans(rock)
Sigma_rock <- cov(rock)

# Generate multivariate normal sample of size 500
set.seed(123)
mv_sample <- mvrnorm(n = 500, mu = mu_rock, Sigma = Sigma_rock)
pairs(mv_sample) # plot multivariate Sample
# Display first rows
head(mv_sample)

```

# QUESTION 3: Random Generation Method (Accept–Reject)
```{r}
# Accept–Reject algorithm function
accept_reject <- function(n){
  
  x <- numeric(n)  # vector to store accepted values
  i <- 1           # counter
  ...
  while(i <= n){
    ...
    # Uniform variable for acceptance
    u <- runif(1)
...
# Generate sample
sample_ar <- accept_reject(1000)

# Plot result
hist(sample_ar, probability = TRUE,
     col = "lightgray",
     main = "Accept–Reject Method: Beta(2,5)",
     xlab = "x")

# Overlay true Beta density
...
# excercise 3.14 using the cholosky method

```{r}
## to do the excercise 3.14 using the cholosky method
...
# using the spectural decomposition
